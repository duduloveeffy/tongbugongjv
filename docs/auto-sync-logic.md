...
### 1. 页面加载时（useEffect - 第277行）
```
页面打开 → 并行执行：
  - loadConfig()        // 加载自动同步配置
  - loadSites()         // 加载所有站点列表
  - loadLogs()          // 加载历史同步日志
  - loadBatchStatus()   // 查询当前是否有运行中的批次
```

**重要**：`loadBatchStatus()` **只查询状态，不触发同步**！
- 调用 `/api/sync/batch-status` (GET)
- 仅用于显示当前批次状态
- 不会创建新批次或触发任何同步操作

### 2. 自动刷新机制（useEffect - 第287行）
```
当存在活跃批次时 → 每10秒自动调用 loadBatchStatus()
  - 目的：实时显示批次进度
  - 不触发新的同步操作
```
