...
### 6. 清理所有运行中任务 (Button - handleCleanupAll)
**位置**: 批次监控卡片
**图标**: XCircle
**文本**: "清理所有运行中任务"
**颜色**: outline with orange (橙色)
**作用**: 一键清理所有过期、卡住、超时的批次

**工作流程**:
```
点击按钮 → 用户确认 →
  POST /api/sync/cleanup →
    遍历所有未完成批次 (pending, fetching, syncing)
    检查是否过期、卡住、超时
    将符合条件的批次标记为 failed
    将运行中的站点标记为 failed
  → 返回清理结果 →
    显示: 过期批次数、卡住批次数、超时站点数
  → 刷新批次状态
```

**清理条件**:
- **过期批次**: 超过过期时间（2小时）
- **卡住批次**: 存在超过15分钟但所有站点都未开始处理
- **超时站点**: 运行超过10分钟的站点
...
| `/api/sync/cleanup` | POST | 清理所有运行中任务 | 点击"清理所有运行中任务" |
| `/api/sync/cleanup` | GET | 查看未完成批次状态 | (可手动调用查看) |
