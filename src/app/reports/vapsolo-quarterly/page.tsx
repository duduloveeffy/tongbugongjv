...
export default function VapsoloQuarterlyReport() {
  ...
  const loadReport = async () => {
    setLoading(true);
    try {
      const response = await fetch('/api/reports/vapsolo/quarterly', {
        method: 'POST',
  ...
  const handleExportExcel = () => {
    ...
    try {
      ...
      // 导出
      XLSX.writeFile(workbook, `Vapsolo季报_${selectedQuarter.year}年Q${selectedQuarter.quarter}.xlsx`);
  ...
  const handlePrint = useReactToPrint({
    contentRef: printRef,
    documentTitle: `Vapsolo季报_${selectedQuarter.year}年Q${selectedQuarter.quarter}`,
    onBeforePrint: async () => {
      toast.info('准备打印...');
    },
    onAfterPrint: async () => {
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-bold">Vapsolo 季度报表</h1>
            <p className="text-sm text-muted-foreground">10个站点季度销量统计（含换算规则）</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
