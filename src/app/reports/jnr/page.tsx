...
export default function JnrMonthlyReport() {
  ...
  const [selectedMonth, setSelectedMonth] = useState({ year: defaultYear, month: defaultMonth });
  ...
  const loadReport = async () => {
    setLoading(true);
    try {
      const response = await fetch('/api/reports/jnr/monthly', {
        method: 'POST',
  ...
  const handleExportExcel = () => {
    ...
    try {
      ...
      // 导出
      XLSX.writeFile(workbook, `JNR月报_${selectedMonth.year}年${selectedMonth.month}月.xlsx`);
  ...
  const handlePrint = useReactToPrint({
    contentRef: printRef,
    documentTitle: `JNR月报_${selectedMonth.year}年${selectedMonth.month}月`,
    onBeforePrint: async () => {
      toast.info('准备打印...');
    },
    onAfterPrint: async () => {
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-bold">JNR 月报</h1>
            <p className="text-sm text-muted-foreground">1个零售站点销量统计（FX182/FL162按10支换算）</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
