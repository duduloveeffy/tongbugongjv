...
export default function VapsoloMonthlyReport() {
  ...
  const loadReport = async () => {
    setLoading(true);
    try {
      const response = await fetch('/api/reports/vapsolo/monthly', {
        method: 'POST',
  ...
  const handleExportExcel = () => {
    ...
    try {
      ...
      // 导出
      XLSX.writeFile(workbook, `Vapsolo月报_${selectedMonth.year}年${selectedMonth.month}月.xlsx`);
  ...
  const handlePrint = useReactToPrint({
    contentRef: printRef,
    documentTitle: `Vapsolo月报_${selectedMonth.year}年${selectedMonth.month}月`,
    onBeforePrint: async () => {
      toast.info('准备打印...');
    },
    onAfterPrint: async () => {
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-bold">Vapsolo 月报</h1>
            <p className="text-sm text-muted-foreground">10个站点销量统计（含换算规则）</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
