...
export async function detectProducts(
  ...
  if (supabase && !skipCache) {
    ...
    console.log(`[Product Detection] Supabase 未配置或跳过缓存查询`);
  }
...
async function fetchFromWooCommerce(
  ...
  const productsToCache: Record<string, unknown>[] = [];
  ...
  const fetchSingleSku = async (sku: string): Promise<void> => {
    ...
    try {
      ...
      console.error(`[Product Detection] Failed to fetch SKU ${sku} (延迟: ${latency}ms):`, errorMessage);
