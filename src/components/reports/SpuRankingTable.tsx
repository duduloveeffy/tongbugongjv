...
interface SpuRankingTableProps {
  data: SpuStats[];
  title?: string;
  showTopN?: number;
...
export function SpuRankingTable({ data, title = 'SPU 排行', showTopN, variant = 'default' }: SpuRankingTableProps) {
  ...
  // 如果设置了 showTopN，则只显示前 N 条
  const displayData = showTopN && !showAll ? sortedData.slice(0, showTopN) : sortedData;
  const hasMore = showTopN && sortedData.length > showTopN;
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          {hasMore && (
            ...
            >
              {showAll ? '收起' : `查看全部 (${sortedData.length})`}
            </button>
          )}
        </div>
      </CardHeader>
          ...
          <Table>
            <TableHeader>
              <TableRow className={styles.header}>
                ...
                <TableHead className="font-semibold text-center">SPU</TableHead>
            ...
            <TableBody>
              {displayData.map((item, index) => (
                <TableRow key={item.spu} className="hover:bg-gray-50">
                  <TableCell className="font-medium text-muted-foreground text-center">
                    {index + 1}
                  </TableCell>
                  <TableCell className="font-medium text-center">{item.spu}</TableCell>
              ...
              <TableRow className="bg-gray-50 font-semibold border-t-2">
                <TableCell colSpan={2} className="text-center">
                  {showAll || !hasMore ? '合计' : `TOP ${showTopN} 合计`}
                </TableCell>
